# Documentation

This directory contains the long-form design, project planning, operations, and
research notes for `nas-csi`.

Docs explain ownership, safety boundaries, and operational intent. They should
not duplicate Rust structs, generated protobuf output, or Kubernetes manifest
contents; the code and deploy files are the source of truth for exact APIs and
fields.

## Design

- [Design index](design/README.md)
- [Architecture](design/architecture.md)
- [VM management](design/vm-management.md)
- [Cluster management](design/cluster-management.md)
- [Configuration](design/configuration.md)
- [Discovery and onboarding](design/discovery.md)
- [Component structure](design/component-structure.md)

## Project

- [Project index](project/README.md)
- [Implementation plan](project/implementation-plan.md)
- [File structure](project/file-structure.md)

## Operations

- [Operations index](operations/README.md)
- [Runbooks](operations/runbooks/README.md)
- [Node maintenance](operations/runbooks/node-maintenance.md)
- [Cluster rebuild](operations/runbooks/cluster-rebuild.md)

## Research

- [Research index](research/README.md)
- [TrueNAS virtiofs integration](research/truenas-virtiofs.md)
- [IaC node VMs on libvirt](research/libvirt-iac-vms.md)
- [TrueNAS VM UI integration](research/truenas-vm-ui-integration.md)
- [k3s cluster lifecycle](research/k3s-lifecycle.md)
- [virtiofsd fork strategy](research/virtiofsd-fork-strategy.md)
- [Rust CSI implementation](research/rust-csi.md)
- [SMB and virtiofs coherency](research/smb-virtiofs-coherency.md)

## Lab

- [Lab notes](lab/README.md)
